...
These PNGs are the polished showcase layer for the docs site. The exact audit trail for values and scenario comparisons still lives in `report.md`, `paired-effects.csv`, and `scenario-analysis.csv`.
...
- [`scripts/generate-benchmark-showcase.py`](../../scripts/generate-benchmark-showcase.py)
...
Refresh the published showcase PNGs with `egaki` from the deterministic SVG blueprints:

```bash
python3 scripts/generate-benchmark-showcase.py
```
...
The docs visuals are published from `docs/images/benchmark-study/` as `egaki`-generated PNG showcase infographics, with deterministic SVG source charts generated beside them for rerendering and auditability. The CSV and markdown artifacts remain the exact source of truth for benchmark values.
