//! Embedded agent skills for gitquarry.
...
const SKILLS: &[EmbeddedSkill] = &[EmbeddedSkill {
    ...
    description: "Core CLI usage guide for repository search, inspection, remote tree/code lookup, and scripting",
    source: GITQUARRY_SKILL_SOURCE,
}];
...
///
/// Gitquarry currently ships only the core skill body. Keeping this separate
/// from `skill_content` preserves Harbor-compatible `skills get <name> --full`
...
pub fn skill_locator(name: Option<&str>) -> Option<String> {
    match name {
        ...
        Some(GITQUARRY_SKILL) => Some(format!("embedded://skills/{GITQUARRY_SKILL}")),
        Some(_) => None,
    }
}
...
fn strip_frontmatter(source: &'static str) -> String {
    ...
    for line in source.lines() {
        ...
        // Harbor supports skills wrapped in a ```skill fence. Gitquarry does
        // not emit that wrapper, but accepting it keeps this parser compatible
